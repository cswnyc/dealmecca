...
export default function LoginPage(): JSX.Element {
  ...
  const handleGoogleSignIn = async (): Promise<void> => {
    ...
    try {
      ...
      if (result) {
        setSuccess('Logged in successfully! Checking account status...');
  ...
  return (
    <AuthShell
      ...
      leftPanelTitle="Welcome Back to DealMecca"
      leftPanelSubtitle="Access your saved contacts, org charts, and exclusive media seller community."
    >
      {error && (
          ...
          <div className="flex items-center justify-between text-flk-body-s">
            <div className="flex items-center">
              <input
                id="remember"
                type="checkbox"
                className="h-4 w-4 text-flk-primary focus:ring-flk-primary border-flk-border-subtle rounded"
              />
              <label htmlFor="remember" className="ml-2 block text-flk-body-s text-flk-text-secondary">
                Remember me
              </label>
            </div>
